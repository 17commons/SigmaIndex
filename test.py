...
btcma = pd.DataFrame(cg.get_coin_market_chart_by_id(id='bitcoin', vs_currency='usd', days=1800),columns=['market_caps'])
...
ethma = pd.DataFrame(cg.get_coin_market_chart_by_id(id='ethereum', vs_currency='usd', days=1800),columns=['market_caps'])
...
usdtma = pd.DataFrame(cg.get_coin_market_chart_by_id(id='tether', vs_currency='usd', days=1800),columns=['market_caps'])
...
usdcma = pd.DataFrame(cg.get_coin_market_chart_by_id(id='usd-coin', vs_currency='usd', days=1800),columns=['market_caps'])
...
busdma = pd.DataFrame(cg.get_coin_market_chart_by_id(id='binance-usd', vs_currency='usd', days=1800),columns=['market_caps'])
...
daima = pd.DataFrame(cg.get_coin_market_chart_by_id(id='dai', vs_currency='usd', days=1800),columns=['market_caps'])
...
# 处理空值，向下移动
index['btc_market_caps']=index['btc_market_caps'].shift(index['btc_market_caps'].isna().sum())
index['date']=index['date'].shift(index['date'].isna().sum())
